...
namespace Ubicacion_Articulos.Vistas
{
    ...
    public partial class CombinarArticuloPedido : ContentPage
    {
        ...
        private async void ListaArt_ItemTapped(object sender, ItemTappedEventArgs e)
        {
            try
            {
                ...
                if (dato != null)
                {
                    SqlCommand cmd = new SqlCommand($"INSERT INTO MovArticPrep (his_codigo,his_codtex,his_codnum,his_adicional,his_tipmov,his_articulo,his_aliva,his_cantidad,his_pend,his_cpreparada) Values ({idpedido},'{item.art_codtex}',{item.art_codnum},'{item.adi_codigo}',6,'{item.art_descri}',0,{dato},{dato},{dato})", ConexionMaestra.con);
                    cmd.ExecuteNonQuery();
