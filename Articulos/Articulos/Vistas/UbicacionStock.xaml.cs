...
namespace Ubicacion_Articulos.Vistas
{
    ...
    public partial class UbicacionStock : ContentPage
    {
        ...
        public UbicacionStock()
        {
            InitializeComponent();
            ColorMostrarLista();
            BindingContext = new VMUbicaciones();
        ...
        private void ColorMostrarLista()
        {
            try
            {
                SqlCommand query = new SqlCommand("Select * from AuxApp_ImpEtiqueta where imp_terminal = " + name + " and imp_tipo = 2", ConexionMaestra.con);
                ...
                {
                    btnListaImprimir.BackgroundColor = Color.FromHex("#E0E0E0");
                    btnListaImprimir.TextColor = Color.Black;
                }
            }
            catch (Exception er)
            {
                //DisplayAlert("Mensaje", "" + er.Message, "OK");
                ConexionMaestra.abrir();
            }
        }
        //private async void ListaArt_ItemTapped(object sender, ItemTappedEventArgs e) //Evento que se dispara cuando clickeo encima de una fila de mi lista 
        //{
        //    //var item = e.Item as MUbicaciones; //Guarto todos mis atributos de mi modelo 'MUbicaciones' en la variable 'item'
        //    var item = e.Item as MEditarUbicacion;
        //    var serializer = JsonConvert.SerializeObject(item);
        //    if (item != null)
        //    {
        //        Preferences.Set("datoUbi", serializer);
        //    }
        //    await Navigation.PushAsync(new DetallesArt()); //Me dirige a mi vista 'DetallesArt'
        //}
        ...
        {
            Navigation.PushAsync(new ListaImprimirUbi());
        }

        private void ListaArt_ItemTapped(object sender, ItemTappedEventArgs e)
        {
            //var item = e.Item as MUbicaciones; //Guarto todos mis atributos de mi modelo 'MUbicaciones' en la variable 'item'
            var item = e.Item as MUbicaciones;
            var serializer = JsonConvert.SerializeObject(item);
            if (item != null)
            {
                Preferences.Set("datoUbi", serializer);
            }
             Navigation.PushAsync(new DetallesArt()); //Me dirige a mi vista 'DetallesArt'
        }
    }
}